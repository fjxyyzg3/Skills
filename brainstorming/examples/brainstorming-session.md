...
6. 用户确认整体设计且明确要生成正式 PRD 后，输出 PRD handoff packet，并用 `Natural Handoff` 推荐 `$to-prd`。
...
- 用户尚未确认设计时，直接创建 PRD、design doc、mockup 或实现代码。
- 把 `$to-prd`、`$to-issues`、`$implement` 或 `$quick-change` 串起来自动执行。
- 只给抽象优缺点，不提供可判断的结构、状态、用户路径或 verification seam。

## PRD Handoff Packet Shape

只有用户确认要进入正式 PRD 时才输出：
