...
- 如果用户确认要生成正式 PRD、requirements 或 implementation-ready spec，最多推荐 `$to-prd` 作为唯一 next skill，并说明 `$to-prd` 会负责写入本地 Markdown artifact 和 feature workspace。
- 如果用户只需要 brainstorming 结果、方案比较或暂不进入文档产出，推荐 `none`。
- 自然确认只进入上一条唯一推荐的 next skill，不能跳过 `$to-prd` 自己的 artifact、scope 和写入确认。
- 不要在本 skill 内自动进入 `$to-issues`、`$analyze`、`$implement` 或 `$quick-change`。
...
- 如果用户确认要进入正式 PRD，已输出 PRD handoff packet，并说明 `$to-prd` 是正式文档产出入口。
