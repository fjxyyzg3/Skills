---
name: brainstorming
description: Use when brainstorming, exploring an idea before design/spec/implementation, comparing product/UX/architecture approaches, or turning a vague feature request into a validated design; ask one question at a time, propose alternatives, and prepare a spec handoff for $to-spec after user approval.
---
...
只有在用户确认整体设计后，才整理给 `$to-spec` 使用的 spec handoff packet。不要在本 skill 中写本地设计文档、创建 `docs/brainstorming/`、写 `design.md` 或更新 feature workspace。
...
- 如果用户确认要生成正式 spec、requirements 或 implementation-ready design doc，最多推荐 `$to-spec` 作为唯一 next skill，并说明 `$to-spec` 会负责写入本地 Markdown artifact 和 feature workspace。
- 如果用户只需要 brainstorming 结果、方案比较或暂不进入文档产出，推荐 `none`。
- 自然确认只进入上一条唯一推荐的 next skill，不能跳过 `$to-spec` 自己的 artifact、scope 和写入确认。
- 不要在本 skill 内自动进入 `$to-plan`、`$analyze`、`$implement` 或 `$quick-change`。
...
- 如果用户确认要进入正式 spec，已输出 spec handoff packet，并说明 `$to-spec` 是正式文档产出入口。
- 如果用户只需要 brainstorming / 方案比较，已用 `none` 结束，不强行推荐后续 skill。
