---
name: diagnose
description: "Use when a concrete Generic or Unreal Engine bug, crash, regression, wrong output, flaky behavior, or performance slowdown needs disciplined diagnosis through Active Repro or Artifact-based Triage, falsifiable hypotheses, targeted probes, and a regression seam."
---
...
1. `DGN-GENERIC-ACTIVE`: 普通 failing test 或 CLI/API/UI bug，当前环境能稳定运行。
   - Expected context: `Generic + Active Repro`。
   - Forbidden action: 加载或输出无关 UE workflow。
   - Pass signal: 精确 failure 被观察后才进入 hypotheses。
2. `DGN-GENERIC-ARTIFACT`: 只有 log、core dump、trace 或截图，无法运行原系统。
   - Expected context: `Generic + Artifact-based Triage`。
   - Forbidden action: 声称已复现或 `RootCauseStatus: confirmed`。
   - Pass signal: facts、inferences、3–5 个 hypotheses、probes 与 missing evidence 分离。
3. `DGN-UE-ACTIVE`: UE PIE、Packaged、Blueprint、cook、network、rendering 或 platform symptom，当前工程可运行。
   - Expected context: `UE + Active Repro`。
   - Forbidden action: 不记录 runtime mode 就开始猜代码。
   - Pass signal: 运行形态、精确症状、复现率和适用 UE references 均有证据。
4. `DGN-UE-ARTIFACT`: 只有 UE log、callstack、CrashContext、minidump、trace、capture 或 asset。
   - Expected context: `UE + Artifact-based Triage`。
   - Forbidden action: 把 artifact inspection 写成“已复现”。
   - Pass signal: 最小缺料与每个 hypothesis 的下一 probe 明确。
5. `DGN-UE-MODE-DRIFT`: 症状只在 Packaged、RHI、network 或特定 platform 出现，但当前只验证了 PIE。
   - Expected context: 保持未验证状态。
   - Forbidden action: 宣称 repair verified。
   - Pass signal: 要求在相同 runtime mode 运行 regression seam。
6. `DGN-PERF-BASELINE`: 性能回退但 map、view、RHI、resolution、scalability 或 sample window 未固定。
   - Expected context: 先固定 baseline。
   - Forbidden action: 先改代码再寻找指标。
   - Pass signal: measurement contract 可重复且能比较 before/after。
7. `DGN-HANDOFF`: repair scope 与正确 regression seam 已可执行。
   - Expected context: 唯一 next skill 为 `$implement`。
   - Forbidden action: 同时给多个 repair entry，或把 architecture observation 变成第二个 next skill。
   - Pass signal: handoff packet 包含 context、root-cause status、seam、repair direction、risk 与 missing evidence。
...
- 用户明确描述 Unreal Engine、Editor、PIE、Cooked、Packaged、Shipping、Blueprint、RHI 等症状，或 repository facts 与症状共同指向 UE runtime path 时，选择 `UE Profile`。
- 其他问题选择 `Generic Profile`。
- Generic Profile 不读取或输出 UE references。
- UE Profile 按需读取同一 skill 目录下的 references：
...
UE Profile 还必须记录正确 runtime mode；运行形态选择见 `references/ue/runtime-modes.md`。
...
- 明确标注“未复现”。
- 建立 `Observed facts / Inferences / Unknowns / Missing evidence` 边界。
- 只判断 artifact 能支持或排除什么，不把 correlation 写成 confirmed cause。

UE Profile 同时记录 map、actor/component、asset path、network role、platform、RHI、build configuration 与生命周期/cook/streaming 时机。
...
UE Profile 读取 `references/ue/probes-and-artifacts.md` 中的生命周期、Reflection/Blueprint、Asset/Cook、Threading、Network 与 Performance dimensions；不要把这些维度无条件输出给 Generic Profile。
...
- 排序 hypothesis 的 prediction 已由 targeted probe、单变量实验或 causal intervention 验证。
- 主要替代假设已被直接 evidence 排除，而不是只因“看起来不像”。
- diagnosis report 记录可复查的 evidence pointer，以及它如何连接 cause、failure 与 regression seam。
...
- UE Profile 的 seam 和 runtime parity 见 `references/ue/regression-seams.md`。
