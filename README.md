...
flowchart LR
  ...
  Analyze --> Worktree["using-worktrees"]
...
| `using-worktrees` | 建立隔离分支/worktree 和 baseline |
...
- 非平凡实现链路优先使用 `to-prd -> to-issues -> analyze -> using-worktrees -> implement -> requesting-code-review -> verification-before-completion`。
